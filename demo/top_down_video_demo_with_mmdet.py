...
def main():
    ...
    """
    parser = ArgumentParser(description='MMAction2 top down video demo')
    ## action
    ...
    for frame_id, cur_frame in enumerate(mmcv.track_iter_progress(video)):
        ...
        for result_ind, result in enumerate(mmdet_results):
            if len(result) == 0:
                continue        
            ...
            for i in range(len(result_filtered)):
                ...
                caption_ex = caption.split(":")[0]
                if (caption_ex == "bottle" or caption_ex == "cell phone" or caption_ex == "person"):
                    # print ("\n caption: ",caption)
                    cv2.rectangle(vis_frame, (left, top), (right, bottom), color=bbox_color, thickness=2)
                    cv2.putText(vis_frame, caption, (int(left), int(top - 7)), cv2.FONT_HERSHEY_SIMPLEX, 0.37, text_color, 1)
